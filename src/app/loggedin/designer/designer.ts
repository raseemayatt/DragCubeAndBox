...
export class DesignerComponent implements OnInit {
  ...
  priceTemplateListbyType: any;
  priceTemplateListDatabyType: any;
  labelImageData: any;
  templateData: any;
  ...
  // getLabelTypeByName(){
  //   this.eslService.getPriceLabelTempatesbyType()
  //     .subscribe(data => {
  //       if (data['responseCode'] == 200) {
  //         this.priceTemplateListbyType = data['objList'];
  //         this.priceTemplateListDatabyType = JSON.stringify(this.priceTemplateListbyType);
  //         console.log(this.priceTemplateListDatabyType);
  //       }
  //     });
  ...
  editTemplate(name) {
    ...
    this.eslService.getPriceLabelTempatesbyName(name)
      .subscribe(data => {
        if (data['responseCode'] == 200) {
          this.labelImageData = data['priceLabelTemplateVO'];
          this.priceTemplateImageData = this.labelImageData.templateJson;
          console.log(this.labelImageData.templateName);
          this.templateName = this.labelImageData.templateName;
          console.log("New " + this.templateName);
          var newst = this.priceTemplateImageData.replace(/\\/g, "");

          var str = JSON.parse(newst);
          // this.coloured.nativeElement.value(str.coloredLabel);

          console.log(str);
          console.log(str.width);
          console.log(str.coloredLabel);
          console.log(str.bgColor.r);
          ...
          console.log(this.imageBackgrnd);
          if (labclr == 'true') {
  ...
  createLabelInfo() {
    ...
    for (let i = 0; i < fullList.length; i++) {
      ...
      if (fullList[i]["fieldType"] === true) {
        //product value
        componentField = {};
        if (fullList[i]["productName"] == 'quantityValue' || fullList[i]["productName"] == 'quantityUnit') {
          componentObj['fields'] = [
  ...
  saveTemplate(activate) {
    ...
    if (templateName.trim() == "") {
      ...
      this.eslService.saveUpdatePriceLabelTempate(paperTypeVal, templateName, labelInfoString, templateType, activate, null)
        .subscribe(data => {
          // this.previewLoading = true;
          if (data['responseCode'] == 200) {
            ...
            this.alertService.success("Template Created!",
              { autoClose: true }
            );
            setTimeout(function () {
